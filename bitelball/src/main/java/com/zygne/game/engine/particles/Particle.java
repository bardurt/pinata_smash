package com.zygne.game.engine.particles;
...
//			var b:uint = color & 0xFF;
    ...
    public void update(BoxCollider container) {
        // update with collision
        if (this.isAlive()) {
            if (this.x <= container.getLeft() || this.x >= container.getRight() - this.widht) {
                this.xv *= -1;
            }
            // Bottom is 480 and top is 0 !!!
            if (this.y <= container.getTop() || this.y >= container.getBottom() - this.height) {
                this.yv *= -1;
            }
        }
        update();
